...
def get_trak_projector(device: torch.device, use_sparse: bool = False):
    """ 
    Get trak projectors with an option to use sparse tensors 
    (see https://github.com/MadryLab/trak for details) 
    """
    try:
        num_sms = torch.cuda.get_device_properties(device.index).multi_processor_count

        # Create a dense or sparse tensor depending on the use_sparse flag
        if use_sparse:
            tensor = torch.zeros(8, 1_000, device=device).to_sparse()
        else:
            tensor = torch.zeros(8, 1_000, device=device)

        import fast_jl
        # Test run to catch at init time if projection goes through
        fast_jl.project_rademacher_8(tensor, 512, 0, num_sms)
        projector = CudaProjector
        print("Using CudaProjector")
    except:
        projector = BasicProjector
...
def collect_grads(dataloader,
    ...
    block_size = 64  # fixed block size for the projectors
    projector_batch_size = 1  # batch size for the projectors
